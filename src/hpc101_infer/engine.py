...
class InferenceEngine:
    def __init__(
        ...
        if self.device.type == "cuda" and config.linear_backend == "int4_reference":
            self._warmup_kernels()
            self._capture_decode_graph()

    def _warmup_kernels(self) -> None:
        """Run dummy forward passes to trigger Triton JIT compilation."""
        with torch.inference_mode():
            pad_id = self.model.config.pad_token_id
            warmup_len = 32
            dummy_ids = torch.full(
                (1, warmup_len), pad_id, dtype=torch.long, device=self.device
            )
            self.cache.reset(1)
            self._batch_size = 1
            prefill_batch = Batch(
                input_ids=dummy_ids,
                positions=torch.arange(warmup_len, device=self.device).unsqueeze(0),
                sequence_lengths=torch.tensor([warmup_len], device=self.device),
                curr_max_seq_len=warmup_len,
                mode="prefill",
            )
            _ = self.model(
                prefill_batch, self.cache,
                logits_positions=torch.tensor([0], device=self.device),
            )
            decode_ids = torch.full(
                (1, 1), pad_id, dtype=torch.long, device=self.device
            )
            decode_batch = Batch(
                input_ids=decode_ids,
                positions=torch.tensor([[warmup_len]], device=self.device),
                sequence_lengths=torch.tensor([warmup_len + 1], device=self.device),
                curr_max_seq_len=warmup_len + 1,
                mode="decode",
            )
            _ = self.model(decode_batch, self.cache, logits_to_keep=1)
            self.cache.reset(1)
            self._batch_size = 0
            torch.cuda.empty_cache()

    def _capture_decode_graph(self):
        """Capture decode-step model forward as a CUDA graph."""
        self._decode_graph_captured = False
        if self.config.max_batch_size != 1:
            return
        try:
            max_seq_len = self.config.max_sequence_length
            self._g_token_ids = torch.zeros(1, 1, dtype=torch.long, device=self.device)
            self._g_positions = torch.zeros(1, 1, dtype=torch.long, device=self.device)
            self._g_seq_lengths = torch.tensor([1], dtype=torch.long, device=self.device)
            static_batch = Batch(
                input_ids=self._g_token_ids,
                positions=self._g_positions,
                sequence_lengths=self._g_seq_lengths,
            ...
            with torch.cuda.stream(s):
                self.cache.reset(1)
                self._batch_size = 1
                _ = self.model(static_batch, self.cache, logits_to_keep=1)
            torch.cuda.current_stream().wait_stream(s)
            self._decode_graph = torch.cuda.CUDAGraph()
            with torch.cuda.graph(self._decode_graph):
                self._g_logits = self.model(static_batch, self.cache, logits_to_keep=1)
            self._decode_graph_captured = True
        except Exception:
            self._decode_graph_captured = False
    ...
    def prefill(
        ...
        with measure_operation(self.device, self.config.synchronize_metrics) as metrics:
            logits = self.model(
                model_input, self.cache, logits_positions=sequence_lengths - 1
            )
        next_logits = logits
    ...
    def decode_step(
        ...
        if self._decode_graph_captured and self._batch_size == 1:
            token_ids = token_ids.to(device=self.device, dtype=torch.long)
            if token_ids.ndim == 1:
                token_ids = token_ids[:, None]
            current_lengths = self.cache.lengths[:1].clone()
            next_lengths = current_lengths + 1
            positions = current_lengths[:, None]
            self._g_token_ids.copy_(token_ids)
            self._g_positions.copy_(positions)
            self._g_seq_lengths.copy_(next_lengths)
    ...
    def generate(self, requests: list[GenerationRequest]) -> list[GenerationOutput]:
        ...
        while scheduler.has_unfinished_requests():
            ...
            active = torch.tensor(
                [s.num_scheduled_tokens > 0 for s in decode_schedule.requests],
            ...
            for index, scheduled in enumerate(decode_schedule.requests):
                if scheduled.num_scheduled_tokens > 0:
