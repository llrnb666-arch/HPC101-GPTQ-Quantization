...
try:
    from hpc101_infer.layers.triton_kernels import fused_dequant_gemm as _triton_fused_gemm
    _TRITON_AVAILABLE = True
except Exception:
    pass


def _use_triton_kernel(inputs: torch.Tensor, padded_in: int) -> bool:
    return (
        _TRITON_AVAILABLE
        and inputs.is_cuda
        and padded_in % 2 == 0
...
class QuantizedLinear(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        inputs_2d = inputs.reshape(-1, orig_shape[-1])
        if self.padded_in_features > self.in_features:
            inputs_2d = F.pad(inputs_2d, (0, self.padded_in_features - self.in_features))
        if _use_triton_kernel(inputs_2d, self.padded_in_features):
            out_2d = _triton_fused_gemm(
                inputs_2d, self.qweight, self.scales, self.group_size,
                zeros=self.zeros, bias=self.bias, symmetric=self.symmetric,
            )
        else:
            weight = dequantize_weight(self.quantized_weight(), dtype=inputs_2d.dtype)
            out_2d = F.linear(inputs_2d, weight, self.bias)
