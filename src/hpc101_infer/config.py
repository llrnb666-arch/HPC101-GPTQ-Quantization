...
class EngineConfig:
    ...
    def from_mapping(cls, raw: Mapping[str, object]) -> EngineConfig:
        ...
        offload_embedding = _optional_boolean(
            raw.get("offload_embedding"),
            "config.engine.offload_embedding",
        )
        if offload_embedding is not None:
            values["offload_embedding"] = offload_embedding
    ...
    def __post_init__(self) -> None:
        ...
        if self.scheduler_backend not in {"static_batch", "continuous_batch"}:
            raise ValueError("scheduler_backend must be static_batch or continuous_batch")
