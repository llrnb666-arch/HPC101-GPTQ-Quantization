    ...
    def forward(
        ...
        if self.offload_embedding:
            hidden_states = F.embedding(
                input_ids.cpu(), self.embed_tokens.weight
        ...
        if logits_positions is not None:
            ...
            hidden_states = hidden_states[batch_idx, logits_positions]
        elif logits_to_keep:
            hidden_states = hidden_states[:, -logits_to_keep:]
        if self.offload_embedding:
            logits = F.linear(
                hidden_states.cpu(), self.embed_tokens.weight
